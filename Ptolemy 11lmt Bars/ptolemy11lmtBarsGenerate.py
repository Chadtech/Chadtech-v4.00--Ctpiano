...
for yit in range(104,127):
	thisTone = makeArray(2.1)
	frequency = tones[(yit)%len(tones)]*25*(2**((yit)/len(tones)))

	print 'TONE:', tones[yit%len(tones)], ', MIDI NUMBER: ', yit

	combineWavs(0,makeTriangleEnharmonicMitDecay(frequency,2,60),thisTone,150.*((float(yit)/3.5))/28.)
	combineWavs(0,makeTriangleEnharmonicMitDecayOn(frequency*4,2,60),thisTone,25.*((float(yit)/3.5))/28.)
	combineWavs(0,makeTriangleEnharmonicMitDecayOn(frequency*8,2,30),thisTone,25.*((float(yit)/3.5))/28.)		
	combineWavs(0,makeTriangleEnharmonicMitDecayOn(frequency*3,2,30),thisTone,75.*((float(yit)/3.5))/28.)
