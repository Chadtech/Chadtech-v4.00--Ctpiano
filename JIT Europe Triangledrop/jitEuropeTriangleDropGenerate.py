...
for yit in range(0,127):
	...
	combineWavs(0,makeTriangleEnharmonic(frequency/2.,2,30),thisTone,200.)
	combineWavs(0,makeTriangleEnharmonic(frequency/4.,2,30),thisTone,200.)
	combineWavs(0,makeTriangleEnharmonic(frequency,2,30),thisTone,400.)
	combineWavs(0,makeTone(frequency/4.,2),thisTone,200.)

	buildFile(thisTone,str(yit).zfill(3)+'.wav')
	os.chdir(os.path.abspath('DKd'))
	buildFile(thisTone,str(yit).zfill(3)+'d'+'.wav')
